...
class DetectLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            pred_score = pred_obj * torch.nn.Hardsigmoid()(pred_cls)
    ...
    def transform_pred_box(self, pred_box):
        ...
        w = torch.exp(pred_box[..., 2]) * self.anchors[:, 0]
        h = torch.exp(pred_box[..., 3]) * self.anchors[:, 1]
...
if __name__ == "__main__":
    ...
    head = YoloHead_DPU(input_size=input_size, in_channels=feat_dims, num_classes=num_classes, anchors=anchors)
