...
class Dataset:
    def __init__(self,yaml_path,phase):
        ...
        self.phase = phase
        self.class_list = self.data_item["CLASS_INFO"]
        self.anchors = self.data_item["ANCHORS"] if "ANCHORS" in self.data_item else None

        self.image_paths = []
        for sub_dir in self.data_item[self.phase.upper()]:
            image_dir = Path(self.data_item["PATH"]) / sub_dir
