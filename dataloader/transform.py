    ...
    def __call__(self, image, boxes, labels):
        ...
        while True:
            ...
            for _ in range(50):
                ...
                if max(h,w) / min(h,w) > 2: # Prevents very thin patches
                    continue


                # If the height and width of the crop is w,h and 
                # We need to find the XminYminXmaxYmax of the crop rectangle
                # In order to do that: We need to do something like:
                # Find Xmin,Ymin --> Add w,h to Xmin,Ymin, and boom we got crop rectangle.
                # But in order to do that, we need to statisfy this equation,
                # Xmin + W <= Width(of the image) --> We do this so that the Crop rectangle doesn't
                # Go beyond the original width and height of the image.
                # That is why we do width - w
                ...
                #This is checking if the boundary boxes center fall inside the rectangle.
                #and the condition is for the rect to be accepted, at least one of the BBox must fall inside the rect
                ...
                ## We need to substract so that the boxes have coordinates relative to the cropped image
